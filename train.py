...
for image in imagelist:
    ...
    bilinearinterp = interpolate.RegularGridInterpolator((heightgrid, widthgrid), LR, method='linear')
    heightgrid = np.linspace(0, height-1, height*2-1)
    widthgrid = np.linspace(0, width-1, width*2-1)
    upscaledLR = bilinearinterp(np.array(np.meshgrid(heightgrid, widthgrid)).T.reshape(-1, 2)).reshape(height*2-1, width*2-1)

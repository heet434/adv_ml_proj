...
for image in imagelist:
    ...
    bilinearinterp = interpolate.RegularGridInterpolator((heightgridLR, widthgridLR), grayorigin, method='linear', bounds_error=False, fill_value=0)
    heightgridHR = np.linspace(0,heightLR-0.5,heightLR*2)
    widthgridHR = np.linspace(0,widthLR-0.5,widthLR*2)
    upscaledLR = bilinearinterp((heightgridHR[:, None], widthgridHR[None, :]))
    ...
    y = ycrcvorigin[:,:,0]
    bilinearinterp = interpolate.RegularGridInterpolator((heightgridLR, widthgridLR), y, method='linear', bounds_error=False, fill_value=0)
    result[:,:,0] = bilinearinterp((heightgridHR[:, None], widthgridHR[None, :]))
    cr = ycrcvorigin[:,:,1]
    bilinearinterp = interpolate.RegularGridInterpolator((heightgridLR, widthgridLR), cr, method='linear', bounds_error=False, fill_value=0)
    result[:,:,1] = bilinearinterp((heightgridHR[:, None], widthgridHR[None, :]))
    cv = ycrcvorigin[:,:,2]
    bilinearinterp = interpolate.RegularGridInterpolator((heightgridLR, widthgridLR), cv, method='linear', bounds_error=False, fill_value=0)
    result[:,:,2] = bilinearinterp((heightgridHR[:, None], widthgridHR[None, :]))
